...
#endif
U8 getCS_infrared(U8* data,int len)
{
    U8 result = 0;
    int i = 0;
    for(i = 0; i < len; i++)
...
U8 adjust_time(U8* msg_byte)
{
	...
	if(act_flag)
	{
		...
		act_msg[10] = getCS_infrared(act_msg,10);
           	WriteComPort(act_msg,12);
		printf("send message:");
...
int adjust_init()
{
	int ret	;
	//sim_900_init();
    ret = OpenComPort(adjust_port, 1200, 8, "1", 'N');
    if (ret < 0) {
...
void * thread_adjust_read(void * argc)
{
    ...
    U8 buf_tty[256];
        while(1)
        {
           ...
           len_tty = ReadComPort_byte(buf_tty,128,fd_adjust);
	...
	U8 data_tail[]={0x1a};
        ...
        while(1)
        {
            ...
            printf("\r\n");
                ...
                len_cmd = WriteComPort(buf_cmd, (len_cmd + 2));
...
int main (int argc, char *argv[])
{
	...
	pthread_create(&p_write,NULL,thread_uart_write,0);
   	pthread_create(&p_read,NULL,thread_uart_read,0);
