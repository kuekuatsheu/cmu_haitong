...
INT32 OpenComPort (INT32 ComPort, INT32 baudrate, INT32 databit,
                   const char *stopbit, char parity,INT32 *fd)
{
    char           *pComPort;
    INT32           retval;
...
//if(retval)
//	printf("NNNNNNNNNNNthe retval is %d\n",retval);
	...
	{
		data_len = 0;
		while(select(fd+1,&fs_read,NULL,NULL,&tv_timeout_in) > 0)
		{
			
			if(mode == 1)//plc read
		    	{
			if(!msg_begin)
			{	read(fd,&data_temp,1); if(data_temp == 0x68){*data_p = data_temp;data_p++;recv_len++;msg_begin = 1;}}
			else if(!get_len)
			{	read(fd,&data_temp,1); get_len = 1;*data_p = data_temp;data_p++;data_len = data_temp ;recv_len++;	if(data_len>256) {printf("message too long error!\r\n"); return;}}
			else
			{	
				if(recv_len < data_len)//376.2 len is the all frame length
				{
				read(fd,&data_temp,1); 
				*data_p = data_temp;
				data_p++;recv_len++;
			...
			}else if(mode == 2)//infrare 645 adjust time
			{
				if(!msg_begin)
				{	read(fd,&data_temp,1);if(data_temp == 0x68){*data_p = data_temp;data_p++;recv_len++;msg_begin = 1;}}
				else if(!get_len)
				{	read(fd,&data_temp,1); *data_p = data_temp; data_p++;recv_len++;if(recv_len == (six45_head_len+1)) {get_len = 1;data_len = data_temp;}}
				else
				{
					if(recv_len < (data_len +9+1+2))
					{
					read(fd,&data_temp,1);
					*data_p = data_temp;
					data_p++;recv_len++;
...
INT32 ReadComPortA (void *data, INT32 datalength,INT32 fd)
{
    INT32           retval = 0;
    int bytes_read;
    int readlen;
...
static INT32 set_port_attr (
                          ...
                          char parity,INT32 fd)              // N(o), O(dd), E(ven)
{
